...
static int user_property_set_event_handler(const int devid, const char *request, const int request_len)
{
    ...
    cJSON *root = NULL, *Alarm = NULL, *PowerSwitch = NULL;
    ...
    Alarm = cJSON_GetObjectItem(root, "alarmtime");
    if (Alarm) {
        struct Alarmtime t;
        t.hrs = cJSON_GetObjectItem(Alarm, "hrs")->valueint;
        t.min = cJSON_GetObjectItem(Alarm, "min")->valueint;
        t.sec = cJSON_GetObjectItem(Alarm, "sec")->valueint;
        Set_Alarmtime(t);
    } 

    PowerSwitch = cJSON_GetObjectItem(root,"alarm_onoff");
    if(PowerSwitch){
        onoff = PowerSwitch->valueint;
        if(onoff){
            Set_Alarm_On();
        }
        else{
